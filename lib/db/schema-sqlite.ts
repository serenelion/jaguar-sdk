...
export const anonymousSession = sqliteTable('AnonymousSession', {
  id: text('id')
    .primaryKey()
    .notNull()
    .$defaultFn(() => crypto.randomUUID()),
  initialPrompt: text('initialPrompt').notNull(),
  sessionData: text('sessionData', { mode: 'json' }),
  createdAt: integer('createdAt', { mode: 'timestamp' }).notNull(),
  convertedUserId: text('convertedUserId').references(() => user.id),
  convertedAt: integer('convertedAt', { mode: 'timestamp' }),
  convertedChatId: text('convertedChatId').references(() => chat.id),
});
